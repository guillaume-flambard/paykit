...
export default async function Account() {
  if (!clerkConfigured) {
    return (
      <main style={wrap}>
        <Link href="/" style={{ fontSize: 13, color: "#7c7c85", textDecoration: "none" }}>← PayKit</Link>
        ...
        <ol style={{ color: "#a5a5ad", lineHeight: 1.8, fontSize: 14, paddingLeft: 18 }}>
          ...
          <li>Copy the API keys, then set them in Vercel (Production + Preview):
            <pre style={{ background: "#0e0e10", border: "1px solid #1f1f23", borderRadius: 8, padding: 12, marginTop: 8, fontSize: 12.5, color: "#cfcfd6", overflowX: "auto" }}>{`NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_...
  ...
  return (
    <main style={wrap}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 24 }}>
        <Link href="/" style={{ fontSize: 13, color: "#7c7c85", textDecoration: "none", flex: 1 }}>← PayKit</Link>
        <UserButton />
      </div>
      ...
      {projects.length === 0 ? (
        <p style={{ color: "#6b6b73", fontSize: 14 }}>No projects yet — create your first one above.</p>
      ) : (
