...
| Track le coût modèle (USD) par événement meter | `meter(userId, event, cost, costUsd?)` — valider costUsd (≥0, ≤1e6) ; `UsageEvent.costUsd` ; analytics `costUsd/revenueUsd/netUsd/marginPct` (revenue = creditsSold × prix unitaire). Colonne postgres `cost_usd` en migration idempotente. | 2026-08-08 paykit | 1 |
